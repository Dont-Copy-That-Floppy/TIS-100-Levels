...
function get_description()
	return { "Create an Algorithum such that IN.A+IN.B is larger than 999, but the 1000x and 100x is output to OUT.A, and the 10x, and 1x is output to OUT.B" }
end
...
function get_streams()
	...
	LSD = {}
	for i = 1,39 do
		inputA[i] = math.random(499, 999)
		inputB[i] = math.random(499, 999)
		MSD[i] = (inputA[i] + inputB[i])/100
		LSD[i] = (inputA[i] + inputB[i])%100
	end
	return {
		{ STREAM_INPUT, "IN.A", 1, inputA },
		{ STREAM_INPUT, "IN.B", 2, inputB },
		{ STREAM_OUTPUT, "MSD", 0, MSD },
		{ STREAM_OUTPUT, "LSD", 3, LSD },
	}
end
